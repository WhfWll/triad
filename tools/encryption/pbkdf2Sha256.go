...
func (pbkdf2Sha256 Pbkdf2Sha256) PasswordEncode(password string, salt string, iterations int) (string, error) {
	// 一共三个参数，分别是原始密码、盐、迭代次数

	// 如果没有设置盐，则使用12位的随机字符串
	if strings.TrimSpace(salt) == "" {
		salt = pbkdf2Sha256.createRandomString(12)
	}
...
// 随机字符串生成函数（不深入讨论）
func (pbkdf2Sha256 Pbkdf2Sha256) createRandomString(len int) string {
...
func (pbkdf2Sha256 Pbkdf2Sha256) PasswordVerify(password string, encoded string) (bool, error) {
	...
	newEncoded, err := pbkdf2Sha256.PasswordEncode(password, salt, i)
	if err != nil {
		return false, err
	}
