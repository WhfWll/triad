            ...
            <el-tree id="tree-option" ref="selectTree" :accordion="accordion" :data="options" :props="props"
                ...
                <span slot-scope="{ data }">
                    <i :class="[data.color != null ? 'ification_col' : '']"
                        :style="{ 'background-color': data.color }"></i>&nbsp;&nbsp;{{ data.label }}
                </span>
            </el-tree>
        </el-option>
    </el-select>
    ...
    methods: {
        ...
        initHandle() {
            if (this.resultValue) {
                if (this.multiple) {
                    // 多选 
                    this.resultValue.forEach(item => this.valueName.push(item.label));
                } else {
                    // 单选
                    this.valueName = this.resultValue.label;
                }
            }
            this.initScroll()
        },
