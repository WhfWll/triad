<template>
    <div class="taskTargetList">
        ...
        <el-table ref="targetTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column prop="risk" label="目标风险">
             ...
             <el-table-column label="漏洞数量" width="280">
                ...
                <template slot-scope="scope">
                    <span class="tag_status tag_danger bug_status" v-if="scope.row.high >99">99+</span>
                    <span class="tag_status tag_danger bug_status"
                        v-else>{{ scope.row.high }}</span>
                    <span class="tag_status tag_warning bug_status" v-if="scope.row.middle >99">99+</span>
                    <span class="tag_status tag_warning bug_status"
                        v-else >{{scope.row.middle}}</span>
                    <span class="tag_status tag_primary bug_status" v-if="scope.row.low >99">99+</span>
                    <span class="tag_status tag_primary bug_status"
                        v-else>{{scope.row.low}}</span>
                    <span class="tag_status tag_success bug_status" v-if="scope.row.low >99">99+</span>
            ...
            <el-table-column prop="isAliveName" label="存活状态">
                <template slot="header">
                        <span class="cursorPointer" @click="clickButton('存活状态')"
                            :class="(formData.isalive !== '' && formData.isalive !== 0) ? 'active' : ''">存活状态<i
                                class="iconfont iconshaixuan"></i>
                        </span>
                    <el-select popper-class="thSelect" style=" width:150px;" v-model="formData.isalive" clearable
                        size="small" ref="alivelist" @change="handlesearchTarget">
                        <el-option v-for="(item,index) in isalivelist" :key="index" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
                </template>
                <template slot-scope="scope">
                    <span>{{scope.row.isAliveName}}</span>
                </template>
            </el-table-column>

            <el-table-column prop="updateTime" label="更新时间" width="200" fixed="right"> 
                <template slot-scope="scope">
        ...
        <CreateReport :isShow="is_show" :type="6" ref="CreateReport"  :dialogVisible = 'dialogVisible' @click="saveCreate()" @clearCreate="clearCreate()"></CreateReport>
    </div>
</template>
    ...
    methods:{
        clickButton(type){
            ...
            case '目标风险':
                ...
                break;
            case '存活状态':
                this.$refs.alivelist.toggleMenu();
                break;
            }
        },
        async getTaskEnum(){
        ...
        async getData(){
            const res = await vulscan.targetList({
                "taskId": this.task_id,  
                "search":this.formData.target_url,  
                "risk":this.formData.riskLevel, 
        ...
        if (Object.prototype.toString.call(row)=='[object Array]' && row.length>1) {
            ...
            if (Object.prototype.toString.call(row)=='[object Array]') {
                row.forEach(element => {
                    ids=ids+ element.id+','
                    targetUrl=targetUrl+ element.target+','
                    batchConfigJson[element.id] = element.target
                });
                ...
                this.$refs.CreateReport.getinit(ids,targetUrl,batchConfigJson);
                    ...
                    batchConfigJson[row.id] = row.target
                    let idNu = row.id+''
             this.$refs.CreateReport.getinit(idNu,row.target,batchConfigJson);
            }
        },
