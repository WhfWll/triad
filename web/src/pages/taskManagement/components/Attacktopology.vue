  ...
  <div class="target_box" id="con_div">
      ...
      <div style="position: relative; z-index: 2">
        <div class="path_row clearfix">
          ...
          <div class="path_msg_item">
            ...
            <span class="spvalue">{{ basicinfo.remote_control_number }}</span>
          </div>
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <div style="text-align: left">
              <div class="vulnstatistics ml">
                <i class="port port1"></i>
                <label for="">高危目标</label>
                <span>{{ riskTarget.high }}</span>
              </div>
              <div class="vulnstatistics ml">
                <i class="port port2"></i>
                <label for="">中危目标</label>
                <span>{{ riskTarget.mid }}</span>
              </div>
              <div class="vulnstatistics ml">
                <i class="port port3"></i>
                <label for="">低危目标</label>
                <span>{{ riskTarget.low }}</span>
              </div>
              <div class="vulnstatistics ml">
                <i class="port port4"></i>
                <label for="">安全目标</label>
                <span>{{ riskTarget.safe }}</span>
              </div>
            </div>
          </el-col>
          <el-col :span="12">
            <div style="text-align: right">
              <div class="vulnstatistics mr">
                <i class="port port4"></i>
                <label for="">已检测目标</label>
                <span>{{ testTarget.finish }}</span>
              </div>
              <div class="vulnstatistics mr">
                <i class="port port3"></i>
                <label for="">检测中目标</label>
                <span>{{ testTarget.running }}</span>
              </div>
              <div class="vulnstatistics mr">
                <i class="port port1"></i>
                <label for="">待检测目标</label>
                <span>{{ testTarget.wait }}</span>
              </div>
              <div class="vulnstatistics mr">
                <i class="port port5"></i>
                <label for="">不存活目标</label>
                <span>{{ testTarget.fail }}</span>
              </div>
            </div>
          </el-col>
        </el-row>
    ...
    <div class="node_pop">
      <div class="test_dialog">
        ...
        <div class="el-dialog__header" v-if="false">
          <!-- <div class="dialog__title clearfix">
            <div class="title_item">
              <i class="micon iconfont iconzhuangtai"></i>
              <label for="" class="lbname">测试目标</label>
              <span class="spvalue" @click="btnTasktargetinfo(nodeinfo)">{{
                nodeinfo.target
              }}</span>
            </div>
            <div class="title_item">
              <i class="micon iconfont iconzhuangtai"></i>
              <label for="" class="lbname">目标风险</label>
              <span class="spvalue">{{ nodeinfo.risk_level }}</span>
            </div>
            <div class="title_item">
              <i
                v-if="nodeinfo.vul_field_icon.verify_success"
                class="micon iconfont iconloudongyanzhengchenggong"
                style="margin-right: 8px; margin-left: 8px"
              ></i>
              <i
                v-if="nodeinfo.vul_field_icon.used_success"
                class="micon iconfont iconloudongliyongchenggong"
                style="margin-right: 8px"
              ></i>
              <i
                v-if="nodeinfo.vul_field_icon.remote_control"
                class="micon iconfont iconloudongyuanchengkongzhi"
              ></i>
              <label for="" class="lbname">{{ nodeinfo.vul_success }}</label>
            </div>
          </div> -->
        </div>
        <div class="el-dialog__body">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <div style="display:flex;justify-content: space-between; align-items: center;margin:10px 10px 20px">
              <div>
                <i class="el-icon-s-promotion"></i>
                <span style="margin-left:10px">{{nodeinfo.targetUrl}}</span>
              </div>
              <div>
                <i class="el-icon-s-release"></i>
                <span style="margin-left:10px">{{nodeinfo.riskEnum}}</span>
              </div>
              <div>
                <i class="el-icon-check"></i>
                <span style="margin-left:10px">{{nodeinfo.verifyEnum}}</span>
              </div>
              <div>
                <i class="el-icon-s-order"></i>
                <span style="margin-left:10px">{{nodeinfo.fen}}</span>
              </div>
              
            </div>
            <el-tab-pane label="节点详情" name="tabs1">
              <div class="tablebg">
                <div class="tablestyle firsttable">
                  <p>
                    <span class="title">业务层</span
                    ><span class="info">{{ nodeinfo.applicationLevel }}</span>
                  </p>
                   <p>
                    <span class="title">支撑层</span
                    ><span class="info">{{ nodeinfo.supportLevel }}</span>
                  </p>
                  <p>
                    <span class="title">服务层</span
                    ><span class="info info2">{{ nodeinfo.serviceLevel }}</span>
                  </p>
                 
                  <p>
                    <span class="title">系统层</span
                    ><span class="info">{{ nodeinfo.systemLevel }}</span>
                  </p>
                  <p>
                    <span class="title">硬件层</span
                    >
                    <span class="info">{{ nodeinfo.hardwareLevel }}</span>
                  </p>
                </div>
              </div>
              <div class="loop_box">
                <div class="part_title" style="font-size: 14px">漏洞信息</div>
                <el-table
                  ref="multipleTable"
                ...
                <el-pagination
                  ...
                  :total="total"
                  @current-change="currentchangevuln"
                >
                </el-pagination>
              </div>
            ...
            <el-tab-pane label="漏洞取证" name="tabs2">
              <h4 style="margin:10px 0 20px">漏洞取证</h4>
              <div v-if="riskTableData">
                <el-table
                  ref="riskTable"
                  :data="riskTableData"
                  style="width: 100%"
                 ...
                 <el-table-column type="expand">
                        <template slot-scope="props">
                            ...
                            <!-- <logonCredentials :detailSelect="detailSelect" :expandData="expandData" :details="props.row" :sqlTable="sqlTable">
                              ...
                              <label for="" class="lbtxt">漏洞名称：
                                  <span style="margin-left:20px">
                                      <span>{{ props.row.vulName }}</span>
                                  </span>
                              </label>
                              <label for="" class="lbtxt">漏洞风险：
                                  <span style="margin-left:20px">
                                      <span>{{ props.row.riskType }}</span>
                                  </span>
                              </label>
                              <label for="" class="lbtxt">漏洞地址：
                                  <span style="margin-left:20px">
                                      <span>{{ props.row.targetUrl }}</span>
                                  </span>
                              </label>
                ...
                @current-change="currentchangeRisk"
                >
               </el-pagination>
              </div>
              <!-- 远程控制 -->
              <h4 style="margin:10px 0 20px">远程控制</h4>
              <div v-if="riskTableData2">
                <el-table
                  ref="riskTable"
                  :data="riskTableData2"
                  style="width: 100%"
                ...
                layout=" total, prev,   next" 
                :total="riskTotalpage2"
               
                @current-change="currentchangeRisk2"
                >
               </el-pagination>
              </div>
            </el-tab-pane>
...
export default ({
  ...
  methods: {
    // 节点详情--start
      //通用节点详情
          async getNodeinfo(){
            console.log('sssss',typeof this.$route.query.id);
            const res = await task.tasktopologymapnodedetail({
                // task_id: this.task_id,
                // node_id:this.node_id
                detailId:"target_"+this.node_id
        ...
        //漏洞取证+ 
         async getPermeateData() {
            ...
            if (dt.code == 200) {
                this.riskTotalpage = dt.data.count||0;
                this.riskTableData = dt.data.result||[];
//                  this.riskTableData = [
//     {
//         "id": 41,
//         "vulName": "Elasticsearch-CVE-2021-22145-敏感信息泄漏漏洞",
//         "riskType": "敏感数据",
//         "riskTypeNum": 0,
//         "targetUrl": "121.36.20.206",
//         "checkResultID": 0,
//         "targetID": ""
//     }
// ]
       ...
       //远程控制
           async getPermeateData2() {
   
            const dt = await task.riskList2({
                taskId: this.$route.query.id , 
                page: this.currentpageRisk2,
                size: this.pageSize3,
                targetId:this.node_id,
            })

            if (dt.code == 200) {
                // this.riskTotalpage2 = 1;
                this.riskTotalpage2 = dt.data.count||0;
                this.riskTableData2 = dt.data.result||[];
//                 this.riskTableData2 = [
//     {
//         "id": 59,
//         "sessionNum": "1694500178",
//         "targetUrl": "192.168.3.80",
//         "route": "1",
//         "remoteControl": "反弹shell",
//         "status": "在线"
//     }
// ];
    ...
    currentchangeRisk (t) {
      ...
      this.getPermeateData()
    },
    currentchangeRisk2 (t) {
      this.currentpageRisk2 = t
      this.getPermeateData2()
    },
    closeDialog () {
             document.querySelector('.node_pop').classList.remove('openAnimate')
        document.querySelector('.node_pop').classList.add('closeAnimate')
    },
    //-------------------------渗透路径 -----------------
    handleShowHide () {
      if (this.show) {
    ...
    svgclick (e) {
      ...
      if(e.target.parentNode.id == 0) {
        this.$message("根节点不可点击")
        return
      } 
      if (e.target.tagName === 'rect') {
        let _id = e.target.parentNode.id
        this.diagGraph.changePoint(_id)
        
       document.querySelector('.node_pop').classList.remove('closeAnimate')
        document.querySelector('.node_pop').classList.add('openAnimate')
        this.node_id = _id
    ...
    async getData (tabname) {
      let that = this
      const res = await task.tasktopologymap({
        taskId: this.target_id
      })
      if (res.code == 200) {
        this.treeloading = false
        this.basicinfo.remote_control_number = res.data.remoteControlNum
        this.basicinfo.progress = res.data.statusEnum
        this.riskTarget = res.data.riskTarget
        this.testTarget = res.data.checkTarget
    ...
    handleClick (tab) { //节点详情、渗透数据切换
      if (tab.name == 'tabs1') {
        this.getNodeinfo()
        this.getNodevuln()
      } else if (tab.name == 'tabs2') {
        this.getPermeateData()
        this.getPermeateData2()
      }
    },
...
.node_pop{
    ...
    background: #fff;
    z-index: 88;
    box-shadow: 0px 8px 32px 1px rgba(76, 122, 227, 0.1200);
