// Package base
// @Author bcy2007  2025/12/23 14:42
package base
...
func (f *FacadeServer) getHTTPHandler(isHttps bool) FacadeConnectionHandler {
	return func(peekConn *BufferedPeekableConn) error {
		var c net.Conn = peekConn
		c.SetDeadline(time.Now().Add(3 * time.Second))
		log.Infof("start to read http request from %s", c.RemoteAddr())
		req, err := ReadHTTPRequestFromBufioReader(bufio.NewReader(c))
		if err != nil {
		...
		reqRaw, err := HttpDumpWithBody(req, true)
		if err != nil {
		...
		for k, v := range f.httpResource.Resources {
			pattern := k
			response := v.Resource
			if MatchAllOfGlob(pattern, token) && response != nil {
				switch response.responseType {
				case HttpResourceType_Raw:
					responseRaw := InterfaceToBytes(response.resource)
					rsp, _, _ := FixHTTPResponse(responseRaw)
					if rsp != nil {
						responseRaw = rsp
					}
					c.Write(rsp)
				...
				case HttpResourceType_Body:
					responseBody := InterfaceToBytes(response.resource)
					var mimeType = "text/html"
					t, _ := filetype.Match(responseBody)
					if t.Extension != "" {
						mimeType = t.MIME.Value
					}
					raw := ReplaceHTTPPacketBody(ReplaceMIMEType([]byte(BodyResponseHeader), mimeType), responseBody, false)
					fixed, _, _ := FixHTTPResponse(raw)
					if fixed != nil {
						raw = fixed
					}
					c.Write(fixed)
				...
				case HttpResourceType_File:
					resource := InterfaceToBytes(response.resource)
					respb := append([]byte(FileResponseHeader), resource...)
					resp, body, err := FixHTTPResponse(respb)
					if err != nil {
