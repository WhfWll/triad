<template>
  <div>
    <div class="main-title">
      <label class="taskSceneBtn">Burpsuite</label>
    </div>
    <div class="tasklist context_box_bg">
      ...
      >
        ...
        <el-table-column v-if="false" prop="executeTypeName" label="任务类型">
          <template slot-scope="scope">
            {{ scope.row.executeTypeName }}
          </template>
        </el-table-column>
        <el-table-column v-if="false" label="任务风险" show-overflow-tooltip>
          <template slot="header">
            <!-- <div style="position: relative;height:10px;"> -->
            <span
              class="cursorPointer"
              @click="clickButton('任务风险')"
              :class="
                formData.risk_level !== '' && formData.risk_level !== 0
                  ? 'active'
                  : ''
              "
              >任务风险<i class="iconfont iconshaixuan"></i>
            </span>
        ...
        <el-table-column v-if="false" prop="" label="风险数量">
          <template slot="header">
            风险数量
            <el-tooltip class="item" effect="dark" placement="right">
              <div slot="content">
              ...
              <i
                ...
                style="position: absolute; top: 0; left: 66px"
              ></i>
            </el-tooltip>
          </template>
          <template slot-scope="scope">
            <span class="tag_status tag_danger bug_status">{{
              scope.row.targetRisk[0]
            }}</span>
            <span class="tag_status tag_warning bug_status">{{
              scope.row.targetRisk[1]
            }}</span>
            <span class="tag_status tag_primary bug_status">{{
              scope.row.targetRisk[2]
            }}</span>
            <span class="tag_status tag_success bug_status">{{
              scope.row.targetRisk[3]
            }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="riskEnum" label="任务风险"> </el-table-column>
  ...
  methods: {
    ...
    async upload (obj) {
      ...
      try {
        ...
        this.$ajax
          .post("/smart/tripartite/burpsuiteupload", formData, {
    ...
    async getData () {
      try {
        const res = await task.taskList2({
    ...
    async btnMultiDeleteTask () {
      ...
      const res = await task.taskDel2({
        burpsuiteIds: _ids.join(",")
      })
      if (res.code == 200) {
        this.$message({
          message: "删除任务成功",
    ...
    async btnDel (scope) {
      const res = await task.taskDel2({ burpsuiteIds: scope.id + '' })
