        ...
        <div class="nodelist div-list">
            <div class="search-box">
                <div class="operationbutton">
                     <xz-button type="primary" @click="newAdd" :disabled="false" size="small" style="margin-right: 8px">添加</xz-button>
                    <del-button :width="170" @click="btnMultiDeleteTemplate" :disabled="!multipleSelection.length"></del-button>
                     <xz-button type="primary" @click="downloadlinux" :disabled="false" size="small">linux agent</xz-button>
                     <xz-button type="primary" @click="downloadwindows" :disabled="false" size="small">windows agent</xz-button>

                </div>
                <div class="serach-condition">
                    <div style="margin-right:20px;display: inline-flex;align-items: center;">
                        <span style="font-size:14px">使用说明</span>
                         <el-tooltip placement="bottom" effect="light">
                            <div slot="content" style="line-height: 1.5; max-width: 400px;">
                                <div style="font-weight: bold; margin-bottom: 5px; font-size: 14px;">1. 启动 Agent 服务</div>
                                <div style="margin-bottom: 8px;">下载对应系统的 Agent 包并解压，在终端/命令行中执行：</div>
                                <div style="margin-bottom: 4px;"><span style="font-weight: bold;">Windows (CMD)：</span>scanner.exe grpc --listen 0.0.0.0:8089</div>
                                <div style="margin-bottom: 4px;"><span style="font-weight: bold;">Windows (PowerShell)：</span>.\scanner.exe grpc --listen 0.0.0.0:8089</div>
                                <div style="margin-bottom: 12px;"><span style="font-weight: bold;">Linux：</span>./scanner grpc --listen 0.0.0.0:8089</div>
                                
                                <div style="font-weight: bold; margin-bottom: 5px; font-size: 14px;">2. 添加节点配置</div>
                                <div style="margin-bottom: 8px;">服务启动后，点击页面“添加”按钮并填写如下信息：</div>
                                <div>- <strong>节点名称：</strong> 自定义名称（例如：扫描节点-1）</div>
                                <div>- <strong>节点IP：</strong> 部署 Agent 机器的 IP 地址</div>
                                <div>- <strong>节点端口：</strong> 8089（需与启动命令中的端口一致）</div>
                            </div>
                            <i class="el-icon-question" style="margin-left: 5px; cursor: pointer; color: #409EFF;"></i>
                        </el-tooltip>
                    </div>
                    <div class="search-text">
                        <el-input placeholder="搜索节点名称" @keydown.enter.native="handlesearch" v-model="search_item.search" class="input-with-select"
            ...
            <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" v-model="Loading" style="width: 100%" height="calc(100% - 102px)"
                @selection-change="handleSelectionChange" @cell-mouse-enter="mouseenter" @cell-mouse-leave="mouseleave">
                <el-table-column type="selection" width="55">
                </el-table-column>
                <el-table-column prop="name" label="节点名称">
                    <template slot-scope="scope">
                        <!-- <el-link @click="btninfo(scope.row)">{{scope.row.name}}</el-link> -->
                        <el-link>{{scope.row.name}}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="ip" label="节点IP"></el-table-column>
                <el-table-column prop="port" label="端口"></el-table-column>
                <el-table-column prop="runningNum" label="任务数量"></el-table-column>
                <el-table-column prop="statusEnum" label="在线状态">
                    <template slot-scope="scope">
                        <span v-if="scope.row.statusEnum === '在线'" style="color: #67C23A; font-weight: 500;">
                ...
                <el-table-column prop="isDisableEnum" label="是否启用"></el-table-column>
                <el-table-column prop="createTime" label="创建时间"></el-table-column>
                
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <div>
                             <el-link :underline="false" class="link_primary" @click="handleEdit(scope.row)">编辑</el-link>
                            <el-link :underline="false" class="link_primary" @click="btnNodeAuthorize(scope,0)"
                                v-if="scope.row.IsDisable === 1">启用</el-link>
                            <el-link :underline="false" class="link_primary" @click="btnNodeAuthorize(scope,1)"
                                v-if="scope.row.IsDisable === 0">禁用</el-link>
              ...
              <div class="bugbasicinfo">
                  <el-form :model="checkIPform" status-icon  ref="ruleFormaddip" :rules="rules" label-width="80px">
                    <el-form-item label="节点名称" prop="name">
                        <el-input v-model="checkIPform.name" size="small" style="width:100%" autocomplete="off" placeholder="请输入节点名称"></el-input>
                    </el-form-item>
                    <el-form-item prop="ip" label="节点IP">
                    <el-input v-model="checkIPform.ip" size="small" style="width:100%" autocomplete="off"
                            placeholder="请输入节点ip"></el-input>
                    </el-form-item>
                    <el-form-item prop="port" label="节点端口">
                    <el-input v-model="checkIPform.port" size="small" style="width:100%" autocomplete="off"
...
export default {
    ...
    created: function () {
        this.$store.state.activefirstMenu = "/node"; 
        this.pageSize = this.commonjs.pageSize;
		this.timermillisec = this.commonjs.timermillisec;
        node.getStatus().then(res => {
            if (res.code === 200) {
                this.isOpen = res.data.status === 0 ? false : true
            } else {
    ...
    methods: {
         async downloadlinux(){
            window.open('/api/smart/system/nodedownload?os=linux')
            // let url = '/smart/system/nodedownload';
            ...
            // this.$message.warning('下载大文件时耗时稍长，请耐心等待')
            // let params = {
            //     // id: row.id
            // }
            // const res = await node.download(params); 
        ...
        async downloadwindows(){
            window.open('/api/smart/system/nodedownload?os=windows')
        },
        // 设置开启 不开启
        ...
        async handleEdit(row) {
            ...
            this.dialogFormVisible = true;
            const res = await node.getNodeInfo({ id: row.id });
            if (res.code === 200) {
                this.checkIPform = {
                    id: res.data.id,
                    name: res.data.name,
        ...
        async getData(notloading){
            ...
            const res = await node.getData(params)
            if (res.code === 200) {
                this.Loading = false;
            ...
            this.$refs['ruleFormaddip'].validate(async (valid) => {
                if(valid){
                    ...
                    if (this.checkIPform.id) {
                        params.id = this.checkIPform.id;
                        res = await node.editNode(params);
                    } else {
                        res = await node.addNode(params);
                    }
                    ...
                    if(res.code == 200){
                        ...
                        this.getData();
                        this.$message({
                            message:res.msg|| (this.checkIPform.id ? '编辑成功' : '新增成功'),
                            type: 'success'
                        });
                    }else{
                        this.$message({
        ...
        async btnNodeAuthorize(scope,flag){ //启用禁用
            const res = await node.nodeauthorize({
                id: scope.row.id,
                isDisable:flag
            });
