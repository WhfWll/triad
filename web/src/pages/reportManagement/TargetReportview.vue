...
<!-- <pre >{{data}}</pre> -->
                <el-collapse v-model="activeName" accordion>
                    <el-collapse-item v-for="(data,index) in data" :key="index" :title="data.targetOverview.target" :name="index+1">
                       <div class="wanghaoYing">
                            <div class="firstpart">
                                        ...
                                        <div class="report-item">
                                            ...
                                            <div class="catalog">
                                                <ul id="catalogUl" style="margin: 0 25px 10px 25px;"> 
                                                    <div  v-for="(catalog,i) in data.catalogParent " :key="i">
                                                        <li class="level1" >
                            ...
                            <div style="background: #fff;margin-top: 25px;">
                                ...
                                <div class="targetOverview" style="padding: 0 25px;" v-if="true">
                                    <table class="firsttable">
                                        <tbody>
                                        <tr>
                                            <td class="nodarkback renwugyArea-tdLeft">测试目标：</td>
                                            <td class="lightback renwugyArea-tdRight" id="task-name">{{ data.targetOverview.target ||''}}</td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="darkback renwugyArea-tdRight" id="task-risk">
                                                <span class="levelcolor " style="width: 8px;"
                                                        :class="[
                                                            {'highcolor':data.targetOverview.risk == '高危'},
                                                            {'middlecolor':data.targetOverview.risk == '中危'},
                                                            {'lowcolor':data.targetOverview.risk == '低危'},
                                                            {'infocolor':data.targetOverview.risk == '安全' || data.targetOverview.risk == '未发现'}
                                                        ]"></span>{{ data.targetOverview.risk }}</td>
                                        </tr>
                                        <tr>
                                            <td class="nodarkback renwugyArea-tdLeft">漏洞分布：</td>
                                            <td class="lightback renwugyArea-tdRight" id="loudongfenbu">
                                                <div><span>漏洞总数：</span><span id="bug-count">{{ data.targetOverview.vulnStat.total }}</span></div>
                                                <div>
                                                    <span>致命漏洞：</span>
                                                    <span id="high-bugCount" class="spanR">{{ data.targetOverview.vulnStat.deadlyNumber }}</span>
                                                    <span>高危漏洞：</span>
                                                    <span id="middle-bugCount"  class="spanR">{{ data.targetOverview.vulnStat.highNumber }}</span>
                                                    <span>中危漏洞：</span>
                                                    <span id="low-bugCount"  class="spanR">{{ data.targetOverview.vulnStat.middleNumber }}</span>
                                                    <span>低危漏洞：</span>
                                                    <span id="info-count">{{ data.targetOverview.vulnStat.lowNumber }}</span></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="darkback renwugyArea-tdRight">
                                                <div><span>验证成功：</span>
                                                    <span id="validateSuccess-count"  class="spanR">{{data.targetOverview.vulnVerify.verifySuccess }}</span>
                                                    <span>利用成功：</span>
                                                    <span id="useSuccess-count" class="spanR">{{data.targetOverview.vulnVerify.useSuccess }}</span>
                                                    <span>未验证：</span>
                                                    <span id="unvalidate-count" class="spanR">{{data.targetOverview.vulnVerify.repairSuccess }}</span>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="nodarkback renwugyArea-tdLeft">测试时间：</td>
                                            <td class="lightback renwugyArea-tdRight" id="test-time">{{ data.targetOverview.createDate }}</td>
                                        </tr> 
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <div class="assetInfo "  style="padding: 0 25px;" v-if="true"> 
                                    <table class="firsttable " style="width: 100%;">
                                        <tbody> 
                                            <tr>
                                                <td class="darkback renwugyArea-tdLeft" >组件/指纹</td>
                                                <td class="darkback renwugyArea-tdRight"  >{{ data.assetInfo.component }}</td>
                                            </tr>
                                            <tr>
                                                <td class="nodarkback renwugyArea-tdLeft"  >服务</td>
                                                <td class="lightback renwugyArea-tdRight"  >{{ data.assetInfo.service }}</td>
                                            </tr>
                                            <tr>
                                                <td class="darkback renwugyArea-tdLeft"  >IP/域名</td>
                                                <td class="darkback renwugyArea-tdRight"  >{{ data.assetInfo.ipOrUrl }}</td>
                                            </tr>
                                            <tr>
                                                <td class="nodarkback renwugyArea-tdLeft"  >操作系统</td>
                                                <td class="lightback renwugyArea-tdRight"   >{{ data.assetInfo.system }}</td>
                                            </tr> 
                                        </tbody>
                                    </table>
                                </div>
                                            ...
                                            <el-table-column type="expand" >
                                                <template slot-scope="scope">
                                                    <div class="displaybox">
                                                        ...
                                                        <p><span class="loopname">payload：</span><span>{{ scope.row.verMsg.payload }}</span></p>
                                                        <p><span class="loopname">请求报文：</span><span v-dompurify-html="highlightedRequest(scope.row.verMsg.request, scope.row.verMsg.payload) "> </span></p>
                                                        <p><span class="loopname">响应报文：</span><span v-dompurify-html="highlightedRequest(scope.row.verMsg.response, scope.row.verMsg.payload_success_flag) "></span></p>
...
export default {
    ...
    methods:{
        async getData(){  
            ...
            if (res.code === 200) {
                ...
                if(Object.prototype.toString.call(this.data) === '[object Array]'){
                    ...
                    //是对象将其变为数组
                    let arr = [];
                    arr.push(JSON.parse(res.data.content));
                    this.data = arr;
                }
        ...
        highlightedRequest(request,payload) {
            ...
            let safeResult = escapeHtml(request);
            if(payload == ''){
                return safeResult
            }
            
            // 对 payload 也进行 HTML 转义，确保与请求报文格式一致
            let safePayload = escapeHtml(payload);
            
            // 1. 尝试直接匹配（转义后的 payload）
            let escapedPayload = this.escapeRegExp(safePayload);
            let replacestr = safeResult.replace(new RegExp(`(${escapedPayload})`, 'g'), '<span style="color: red;font-weight:700">$1</span>');
            
            // 2. 如果直接匹配失败，尝试 URL 编码后匹配（标准编码，空格为 %20）
            if (replacestr === safeResult) {
                let encodedPayload = encodeURIComponent(payload);
                let escapedEncodedPayload = this.escapeRegExp(encodedPayload);
                replacestr = safeResult.replace(new RegExp(`(${escapedEncodedPayload})`, 'g'), '<span style="color: red;font-weight:700">$1</span>');
            }
