  ...
  <div>
    ...
    <div class="tasklist context_box_bg">
      ...
      >
        ...
        <el-table-column prop="vulName" label="漏洞名称"> </el-table-column>
        <el-table-column prop="vulTypeName" label="漏洞类型"> </el-table-column>
        <el-table-column prop="vulRiskName" label="漏洞风险"> </el-table-column>
        <el-table-column prop="vulLocation" label="漏洞位置"> </el-table-column>
     ...
     <!-- 漏洞详情 -->
            <vulnmsginfo :typeNameWang='typeNameWang'  v-model="sendVal" :vulninfo=vulninfo :task_id="task_id"   >
            </vulnmsginfo>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getData () {
      try {
        const res = await traffic.clearTrafficlog2({
        ...
        if (res.code == 200) {
          this.tableData = res.data.assetVulInfo
    ...
    async btnMultiDeleteTask () {
      ...
      let _ids = this.multipleSelection.map(item => item.vulID)

      const res = await traffic.clearTrafficlogDel({
        ids: _ids.join(",")
      })
      if (res.code == 200) {
        this.$message({
    ...
    async btnDel (scope) {
      ...
      const res = await traffic.clearTrafficlogDel({ ids: scope.vulID + '' })
